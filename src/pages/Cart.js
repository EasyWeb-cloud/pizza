...
const Cart = () => {
  ...
  const onClearCart = () => {
    if(window.confirm('Do you really want to clear cart?')) dispatch(clearCart())
  }

  const onRemoveItem = id => {
    if(window.confirm('Do you really want to delete this pizza from cart?')) dispatch(removeCartItem(id))
  }
...
<path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
            ...
            <div className="content__items">
              {
                addedPizzas.map(obj => <CartItem onMinus={onMinusItem} onPlus={onPlusItem} id={obj.id} onRemove={onRemoveItem} totalCount={items[obj.id].items.length} totalPrice={items[obj.id].totalPrice} name={obj.name} type={obj.type} size={obj.size}/>)
              }
            </div>
